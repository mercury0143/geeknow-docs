...
```bash
curl https://api.example.com/v1/video/generations/task-video-abc123 \
...
resp = requests.get(
    "https://api.example.com/v1/video/generations/task-video-abc123",
...
```json
{
  ...
  "data": {
    "task_id": "task-video-abc123",
    "status": "succeeded",
    "url": "https://api.example.com/v1/videos/task-video-abc123/content",
    "format": "mp4",
    "metadata": {
      "duration": 5,
      "width": 1280,
...
```bash
curl https://api.example.com/v1/videos/task-video-abc123 \
  -H "Authorization: Bearer YOUR_API_KEY"
```
