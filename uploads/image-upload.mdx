...
```bash
curl -X POST https://api.example.com/api/upload/presign \
...
resp = requests.post(
    "https://api.example.com/api/upload/presign",
    headers={
...
```javascript
const response = await fetch("https://api.example.com/api/upload/presign", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
