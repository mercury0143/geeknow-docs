...
```bash
curl -X POST https://apidoc.geeknow.top/v1/responses \
...
```bash
curl -N -X POST https://apidoc.geeknow.top/v1/responses \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o",
...
```bash
curl -X POST https://apidoc.geeknow.top/v1/responses/compact \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o",
