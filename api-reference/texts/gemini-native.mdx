...
```bash
curl -X POST https://apidoc.geeknow.top/v1beta/models/gemini-2.0-flash:generateContent \
...
```bash
curl -N -X POST https://apidoc.geeknow.top/v1beta/models/gemini-2.0-flash:streamGenerateContent \
  -H "x-goog-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "contents": [
