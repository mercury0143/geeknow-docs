...
```bash
curl -X POST https://apidoc.geeknow.top/v1/video/generations \
...
resp = requests.post(
    "https://apidoc.geeknow.top/v1/video/generations",
    headers={
...
```javascript
const response = await fetch("https://apidoc.geeknow.top/v1/video/generations", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
