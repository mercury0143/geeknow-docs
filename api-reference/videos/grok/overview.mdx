...
- `grok-imagine-video` / `grok-imagine-video-1.5-preview` 使用 `application/json`；`grok-imagine-video` 通过 `image` 或 `images` 传入参考图，`grok-imagine-video-1.5-preview` 通过 `images` 传入一张或多张参考图。
...
| `grok-imagine-video-1.5-preview` | JSON | 按字符串传入秒数 | 参考图传 `images`，支持多张 |
...
| Grok Imagine 图生视频 | JSON `image` 或 `images` | `grok-imagine-video-1.5-preview` 使用 `images`；`grok-imagine-video` 可使用 `image` 或 `images` |
