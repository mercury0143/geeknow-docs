...
- `grok-imagine-video` / `grok-imagine-video-1.5-preview` 使用 `application/json`；`grok-imagine-video` 通过 `image` 或 `images` 传入参考图，`grok-imagine-video-1.5-preview` 通过 `images` 传入参考图，最多 `7` 张。
...
| 模型 | 请求格式 | 时长规则 | 参考图规则 |
| --- | --- | --- | --- |
| `grok-imagine-video` | JSON | `4` 到 `15` 秒 | 单图传 `image`，多图传 `images`，最多 `7` 张 |
| `grok-imagine-video-1.5-preview` | JSON | `4` 到 `15` 秒 | 参考图传 `images`，最多 `7` 张 |
...
| Grok Imagine 图生视频 | JSON `image` 或 `images` | `grok-imagine-video-1.5-preview` 使用 `images`；`grok-imagine-video` 可使用 `image` 或 `images`，参考图最多 `7` 张 |
...
| `resolution` | `480P`、`720P` | Grok Imagine 1.5 支持 `480P`、`720P` |
| `seconds` | 字符串形式的 `4` 到 `15` 秒 | Grok Imagine 支持按秒提交，建议按字符串传入 |
| `images` | 最多 `7` 张 | 多图参考时使用；`grok-imagine-video-1.5-preview` 统一使用 `images` |
