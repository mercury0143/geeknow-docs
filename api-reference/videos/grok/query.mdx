...
Grok 系列任务提交后，通过 `GET /v1/videos/{task_id}` 轮询状态。`grok-video-1.5` / `grok-video-3` 与 Grok Imagine 系列共用同一组查询与下载接口。
...
任务对应的模型名。部分 Grok 任务会返回渠道侧归一化后的模型名，例如提交 `grok-imagine-video-1.5-preview`，完成响应里返回 `grok-imagine-video-1.5`。
</ResponseField>
