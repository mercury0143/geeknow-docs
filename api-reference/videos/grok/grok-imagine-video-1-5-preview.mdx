...
`grok-imagine-video-1.5-preview` 是 Grok Imagine 线路的 1.5 Preview 模型。它与 `grok-video-1.5` / Pro / Max 不同：本模型使用 `application/json` 请求体，参考图通过 JSON 字段传递。
...
任务对应的模型名。查询完成时可能返回归一化后的模型名，例如 `grok-imagine-video-1.5`。
</ResponseField>
