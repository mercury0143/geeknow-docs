...
- 图生视频使用 `images` 数组传入参考图，最多 `7` 张。
- 当前 1.5 规范统一使用 `images` 字段传入参考图。
...
目标秒数。支持 `4` 到 `15` 秒；建议按字符串传入，例如 `"4"`。
</ParamField>
...
参考图 URL 数组，最多 `7` 张。请传入上游可访问的图片地址，例如 `["https://example.com/reference-1.png", "https://example.com/reference-2.png"]`。
</ParamField>
