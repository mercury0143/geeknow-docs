...
- 图生视频使用 `images` 数组传入参考图。
- 当前 1.5 规范支持多张参考图，统一使用 `images` 字段。
...
参考图 URL 数组，支持传入多张图片。请传入上游可访问的图片地址，例如 `["https://example.com/reference-1.png", "https://example.com/reference-2.png"]`。
</ParamField>
...
| 图生视频 / 多图参考 | 在文生字段基础上追加 `images` |

`images` 应传入上游可访问的图片 URL 数组；只有一张参考图时，也按 `images: ["https://..."]` 传入。不要把本地文件路径直接作为 JSON 字段提交。
