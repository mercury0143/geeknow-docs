...
| Grok Video | `grok-video-1.5`、`grok-video-3`、`grok-imagine-video-1.5-preview` | [概览](/api-reference/videos/grok/overview) | `/v1/videos` | 文生视频、首帧生视频、首尾帧、参考生视频 | multipart `input_reference` 或 JSON `image` / `images` |
...
| 项目 | 说明 |
| --- | --- |
| 模型示例 | `grok-video-1.5`、`grok-video-1.5-pro`、`grok-video-1.5-max`、`grok-video-3`、`grok-video-3-pro`、`grok-video-3-max`、`grok-imagine-video-1.5-preview` |
| 推荐入口 | `POST /v1/videos` |
| 文档页面 | [Grok 视频概览](/api-reference/videos/grok/overview) |
| 常见字段 | `prompt`、`seconds`、`aspect_ratio`、`size` |
| 参考图 | `grok-video-1.5` / `grok-video-3` 使用 multipart `input_reference`；`grok-imagine-video-1.5-preview` 使用 JSON `image` / `images` |
