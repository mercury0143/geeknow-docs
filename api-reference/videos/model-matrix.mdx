...
| MiniMax H3 | `minimax-h3-768p`、`minimax-h3-2k` | [概览](/api-reference/videos/minimax-h3/overview) | `/v1/videos` | 文生视频、图生视频、首尾帧、参考音频 | JSON `referenceImages` / `first_image` / `last_image` / `referenceAudios`；两个模型能力一致，差异是 768P / 2K 清晰度 |
...
| 项目 | 说明 |
| --- | --- |
| 模型示例 | `minimax-h3-768p`、`minimax-h3-2k` |
| 兼容入口 | `minimax-h3`，按 `resolution`、`video_resolution` 或 `size` 分流到 768P / 2K 实际模型 |
| 推荐入口 | `POST /v1/videos` |
| 文档页面 | [MiniMax H3 视频概览](/api-reference/videos/minimax-h3/overview) |
| 768P 档 | `minimax-h3-768p`；适合草稿预览、批量生成、移动端分发和常规清晰度任务 |
| 2K 档 | `minimax-h3-2k`；适合高清成片、大屏展示、细节保留和后期裁切 |
| 差异边界 | 两档请求字段和生成能力一致；2K 只影响输出清晰度，不代表不同的生成模式 |
| 常见字段 | `prompt`、`duration`、`ratio`、`referenceImages`、`first_image`、`last_image`、`referenceAudios` |
| 清晰度 | `768P`、`2K`；未明确选择时默认按 `768P` 处理 |
| 时长规则 | `duration` 支持 `4` 到 `15` 秒，整数秒，按秒计费 |
| 画幅比例 | 当前项目支持 `16:9`、`9:16`、`1:1`、`21:9`、`4:3`、`3:4` |
| 参考素材 | `503`：最多 5 张参考图、0 段参考视频、3 段参考音频；参考音频需要搭配至少 1 张参考图 |
| 当前边界 | 当前文档不覆盖 `minimax-h3-pro-*` 参考视频模型 |
...
| MiniMax H3 | 支持 | 支持 | 支持 | 支持参考图和参考音频；当前不覆盖参考视频 Pro 档 | 原生音频能力，前端不透出关闭开关 |
...
| `grok-imagine-video-1.5-preview` | Grok Imagine | Grok Imagine 1.5 Preview；JSON 请求；当前仅建议单图 `image`；`resolution` 支持 `480P` / `720P`；`prompt` 最多 `4096` 个字符。 |
| `minimax-h3` | MiniMax H3 | 前端单入口；按 `resolution`、`video_resolution` 或 `size` 分流到 `minimax-h3-768p` 或 `minimax-h3-2k`；未明确选择时默认 768P。 |
| `minimax-h3-768p` | MiniMax H3 | 768P 按秒计费模型；`duration` 支持 `4` 到 `15` 秒；适合草稿、批量和常规清晰度任务。 |
| `minimax-h3-2k` | MiniMax H3 | 2K 按秒计费模型；参数与 `minimax-h3-768p` 一致；适合高清成片、大屏展示和后期裁切。 |
...
3. [Omni 视频模型概览](/api-reference/videos/omni/overview)
4. [Veo 视频概览](/api-reference/videos/veo/overview)
5. 如果你要做图生视频，先确认目标模型需要的是 `image`、`images`、`input_reference`、`referenceImages` 还是 `first_frame_image` / `last_frame_image`。
6. 如果你明确是 Kling 官方格式接入，再看 `/kling/v1/videos/*` 这一组路由。
