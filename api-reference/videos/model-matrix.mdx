...
| Grok Imagine | `grok-imagine-video`、`grok-imagine-video-1.5-preview` | [概览](/api-reference/videos/grok/overview) | `/v1/videos` | 文生视频、图生视频、参考图视频 | JSON `image` / `images`；1.5 Preview 使用 `images` 支持多参考图 |
...
| 参考图 | JSON `image` / `images`，其中 `grok-imagine-video-1.5-preview` 使用 `images` 支持多参考图 |
| 时长规则 | 支持自定义秒数，最小 `1s` |
...
| `grok-imagine-video-1.5-preview` | Grok Imagine | Grok Imagine 1.5 Preview；JSON 请求；使用 `images` 支持多参考图；`resolution` 支持 `480P` / `720P`；`prompt` 最多 `4096` 个字符。 |
