...
| MiniMax H3 | `minimax-h3-768p`、`minimax-h3-2k` | [概览](/api-reference/videos/minimax-h3/overview) | `/v1/videos` | 文生视频、图生视频、首尾帧、参考音频 | JSON `referenceImages` / `first_image` / `last_image` / `referenceAudios` |
...
| 参考素材 | 参考图最多 5 张；参考音频最多 3 段公网 URL，且需要搭配至少 1 张参考图 |
...
| `minimax-h3` | MiniMax H3 | 前端单入口；按 `resolution`、`video_resolution` 或 `size` 分流到 `minimax-h3-768p` 或 `minimax-h3-2k`。 |
| `minimax-h3-768p` | MiniMax H3 | 768P 按秒计费模型；JSON 请求；`duration` 支持 `4` 到 `15` 秒；参考图使用 `referenceImages`，首尾帧使用 `first_image` / `last_image`。 |
| `minimax-h3-2k` | MiniMax H3 | 2K 按秒计费模型；参数与 `minimax-h3-768p` 一致，适合需要更高清晰度的生成任务。 |
