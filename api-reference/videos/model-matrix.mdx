...
| Grok Video 1.5 | `grok-video-1.5`、`grok-video-1.5-pro`、`grok-video-1.5-max` | [Grok Video 1.5](/api-reference/videos/grok/grok-video-1-5) | `/v1/videos` | 首帧生视频、首尾帧、参考生视频；文生能力依渠道而定 | multipart `input_reference`，参考生视频/多图参考场景最多 1 张 |
| Grok Video 3 | `grok-video-3`、`grok-video-3-pro`、`grok-video-3-max` | [Grok Video 3](/api-reference/videos/grok/grok-video-3) | `/v1/videos` | 文生视频、首帧生视频、首尾帧、参考生视频 | multipart `input_reference`，支持重复字段传多图 |
| Grok Imagine | `grok-imagine-video`、`grok-imagine-video-1.5-preview` | [概览](/api-reference/videos/grok/overview) | `/v1/videos` | 文生视频、图生视频、参考图视频 | JSON `image` / `images`；1.5 Preview 当前仅单图 |
...
| 项目 | 说明 |
| --- | --- |
| 模型示例 | `grok-video-1.5`、`grok-video-1.5-pro`、`grok-video-1.5-max` |
| 推荐入口 | `POST /v1/videos` |
| 文档页面 | [Grok Video 1.5](/api-reference/videos/grok/grok-video-1-5) |
| 常见字段 | `prompt`、`seconds`、`aspect_ratio`、`size` |
| 参考图 | 使用 multipart `input_reference`；首尾帧按顺序传首帧、尾帧；参考生视频/多图参考场景最多 1 张 |
| 时长规则 | Pro 固定 `10s`，Max 固定 `15s`；客户端兼容层可把基础模型名按时长解析到 Pro / Max |
| 响应模型名 | 创建或查询响应中的 `model` 可能返回渠道侧归一化后的模型名；后续查询以 `id` / `task_id` 为准 |
| 注意事项 | 文生能力依账号分组和渠道配置而定 |
...
| 推荐入口 | `POST /v1/videos` |
| 文档页面 | [Grok Video 3](/api-reference/videos/grok/grok-video-3) |
| 常见字段 | `prompt`、`seconds`、`aspect_ratio`、`size` |
| 参考图 | 使用 multipart `input_reference`，可重复字段传多张参考图 |
| 时长规则 | Pro 固定 `10s`，Max 固定 `15s`；客户端兼容层可把基础模型名按时长解析到 Pro / Max |
| 响应模型名 | 创建或查询响应中的 `model` 可能返回渠道侧归一化后的模型名；后续查询以 `id` / `task_id` 为准 |
...
| 参考图 | JSON `image` / `images`，其中 `grok-imagine-video-1.5-preview` 当前仅建议传 `image` |
| 时长规则 | 支持自定义秒数，最小 `1s` |
...
| `grok-video-1.5` | Grok Video | Grok 1.5 基础模型名；客户端兼容层可按 `seconds=10/15` 解析到 Pro / Max；multipart `input_reference`；参考生视频/多图参考场景最多 1 张参考图。 |
| `grok-video-1.5-pro` | Grok Video | Grok 1.5 Pro；固定 `10s`；支持文生视频、首帧生视频、首尾帧、参考生视频；参考生视频/多图参考场景最多 1 张参考图。 |
| `grok-video-1.5-max` | Grok Video | Grok 1.5 Max；固定 `15s`；支持文生视频、首帧生视频、首尾帧、参考生视频；参考生视频/多图参考场景最多 1 张参考图。 |
| `grok-imagine-video` | Grok Video | Grok Imagine Video；JSON 请求；单图传 `image`，多图传 `images`；`resolution` 支持 `480P` / `720P`；`prompt` 最多 `4096` 个字符。 |
| `grok-imagine-video-1.5-preview` | Grok Video | Grok Imagine 1.5 Preview；JSON 请求；当前仅建议单图 `image`；`resolution` 支持 `480P` / `720P`；`prompt` 最多 `4096` 个字符。 |
| `grok-video-3` | Grok Video | Grok 3 基础模型名；客户端兼容层可按 `seconds=10/15` 解析到 Pro / Max；multipart `input_reference`。 |
