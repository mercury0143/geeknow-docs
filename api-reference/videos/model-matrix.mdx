...
| `manxue-2.5` | manxue-2.5 | JSON 请求；`duration` 支持 `4` 到 `29` 秒；`resolution` 支持 `480p`、`720p`、`1080p`、`4k`；参考图、参考视频、参考音频分别通过 `referenceImages`、`referenceVideos`、`referenceAudios` 传入。 |
...
4. [Omni 视频模型概览](/api-reference/videos/omni/overview)
5. [Veo 视频概览](/api-reference/videos/veo/overview)
6. 如果你要做图生视频，先确认目标模型需要的是 `image`、`images`、`input_reference`、`referenceImages` 还是 `first_frame_image` / `last_frame_image`。
7. 如果你明确是 Kling 官方格式接入，再看 `/kling/v1/videos/*` 这一组路由。
