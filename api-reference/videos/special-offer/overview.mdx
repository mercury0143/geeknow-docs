...
特惠渠道系列当前包含 `manxue-2` 视频模型。该模型通过 `/v1/videos` 创建异步任务，通过 `/v1/videos/{id}` 查询任务状态和结果。

<CardGroup cols={2}>
  <Card title="manxue-2 创建任务" icon="clapperboard" href="/api-reference/videos/special-offer/generation">
...
| --- | --- | --- | --- | --- | --- |
| manxue-2 | `manxue-2` | JSON | `4` 到 `15` 秒 | `480P`、`720P` | 图片 URL 放入 `content[].image_url.url` |
