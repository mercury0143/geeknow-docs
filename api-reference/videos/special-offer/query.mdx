---
title: manxue-2.0.0 查询视频任务
description: 使用 `GET /v1/videos/{id}` 查询 manxue-2.0.0 视频任务状态。
api: "GET /v1/videos/{id}"
---

# manxue-2.0.0 查询视频任务

这个接口用于查询 `manxue-2.0.0` 异步视频任务状态。创建任务接口返回 `id` 后，可通过该接口轮询状态并读取结果地址。
...
- [特惠渠道系列概览](./overview)
- [manxue-2.0.0 创建视频任务](./generation)
