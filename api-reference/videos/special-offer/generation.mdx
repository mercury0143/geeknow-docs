---
title: manxue-2.0.0 视频生成
description: 使用 `POST /v1/videos` 调用 `manxue-2.0.0` 提交 JSON 视频生成任务。
api: "POST /v1/videos"
---

# manxue-2.0.0

`manxue-2.0.0` 使用 JSON 请求体提交异步视频任务，支持文生视频、首帧生视频、首尾帧和多参考图生成。
...
公网图片 URL。`manxue-2.0.0` 不接收 Base64 data URI。
</ParamField>
...
- 该模型归属特惠渠道系列，模型名固定使用 `manxue-2.0.0`。

## 相关接口

- [特惠渠道系列概览](./overview)
- [manxue-2.0.0 任务查询](./query)
