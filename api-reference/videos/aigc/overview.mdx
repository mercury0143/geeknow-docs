...
国产视频模型指腾讯 VOD AIGC 生视频渠道下的一组模型，统一通过网关视频接口调用，不直接调用云厂商 Action API。
...
1. 发往上游时，规范化为上游需要的 `ModelName` / `ModelVersion`，例如 `Kling`、`Vidu`、`GV`、`OS`、`Hailuo`、`Hunyuan`、`Mingmou`、`SV`、`JV`。
2. 计费时，根据模型、场景、分辨率、参考输入、音频开关、错峰等参数拼接内部计费模型键，用来匹配默认价格。
...
组合计费模型可以直接作为 `model` 传入。网关会自动还原上游模型，并补齐相关计费参数。
...
| `Kling` | `Kling-1.6`、`Kling-2.0`、`Kling-2.1`、`Kling-2.5`、`Kling-2.6`、`Kling-3.0`、`Kling-3.0-Omni`、`Kling-O1` | 文生视频、图生视频、动作控制、数字人、对口型、多镜头扩展 | 需要动作控制、数字人、对口型、音频开关或 `ExtInfo` 透传时，优先看 `Kling`。 |
...
| `metadata` | 扩展参数，建议把上游原生输出配置放入 `metadata.output_config` |

顶层 `seconds` / `duration` 目前仍然生效，不是只能放进 `metadata`。复杂场景建议显式传 `metadata.output_config.resolution` 和 `metadata.output_config.aspect_ratio`，减少上游自动推断带来的歧义。
...
| `video_url` | 参考视频 URL，网关会自动转换为 `FileInfos` |
| `file_infos` | 原生 `FileInfos` 透传，高级用法 |
| `output_config` | `AigcVideoOutputConfig`，支持 snake_case 字段 |
| `ext_info` | 原生 `ExtInfo` 字符串透传，高级用法 |

### `output_config` 字段映射

`metadata.output_config` 支持 snake_case 自动映射到上游官方字段：
...
- 顶层 `image` / `images` / `input_reference` 会自动转换为 `FileInfos`。
...
以下为网关内置默认单价，单位为元 / 秒，用于计费模型键匹配。实际以账户侧开通能力与结算为准。
