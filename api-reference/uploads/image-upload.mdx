...
```bash
curl -X POST https://apidoc.geeknow.top/api/upload/presign \
...
resp = requests.post(
    "https://apidoc.geeknow.top/api/upload/presign",
    headers={
...
```javascript
const response = await fetch("https://apidoc.geeknow.top/api/upload/presign", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
