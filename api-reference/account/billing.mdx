...
```bash
curl https://apidoc.geeknow.top/api/usage/token/ \
...
resp = requests.get(
    "https://apidoc.geeknow.top/api/usage/token/",
...
```javascript
const response = await fetch("https://apidoc.geeknow.top/api/usage/token/", {
  headers: { Authorization: "Bearer YOUR_API_KEY" },
});
